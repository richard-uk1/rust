...
~~~
let (chan, port) = serial::pipe_stream();
...
pub mod serial {
    ...
    use flatteners::{DeserializingUnflattener, SerializingFlattener};
    use flatteners::{deserialize_buffer, serialize_value};
    use bytepipes::{ReaderBytePort, WriterByteChan};
    use bytepipes::{PipeBytePort, PipeByteChan};
    ...
    pub fn pipe_stream<T: Serializable<DefaultSerializer>
                          ...
                          ) -> (PipeChan<T>, PipePort<T>) {
        let (chan, port) = pipes::stream();
        return (pipe_chan(move chan), pipe_port(move port));
    }
...
pub mod pod {
    ...
    use core::io::{Reader, Writer};
    use core::pipes::{Port, Chan};
    use flatteners::{PodUnflattener, PodFlattener};
    use bytepipes::{ReaderBytePort, WriterByteChan};
    use bytepipes::{PipeBytePort, PipeByteChan};

    pub type ReaderPort<T: Copy Send, R> =
        FlatPort<T, PodUnflattener<T>, ReaderBytePort<R>>;
    pub type WriterChan<T: Copy Send, W> =
        FlatChan<T, PodFlattener<T>, WriterByteChan<W>>;
    pub type PipePort<T: Copy Send> =
        FlatPort<T, PodUnflattener<T>, PipeBytePort>;
    pub type PipeChan<T: Copy Send> =
    ...
    /// Create a pair of `FlatChan` and `FlatPort`, backed by pipes
    pub fn pipe_stream<T: Copy Send>() -> (PipeChan<T>, PipePort<T>) {
        let (chan, port) = pipes::stream();
        return (pipe_chan(move chan), pipe_port(move port));
    }
...
pub mod flatteners {
    ...
    pub struct PodUnflattener<T: Copy Send> {
        bogus: ()
    }

    pub struct PodFlattener<T: Copy Send> {
        bogus: ()
    }

    pub impl<T: Copy Send> PodUnflattener<T>: Unflattener<T> {
        fn unflatten(&self, buf: ~[u8]) -> T {
            assert size_of::<T>() != 0;
    ...
    pub impl<T: Copy Send> PodFlattener<T>: Flattener<T> {
        fn flatten(&self, val: T) -> ~[u8] {
            assert size_of::<T>() != 0;
    ...
    pub impl<T: Copy Send> PodUnflattener<T> {
    ...
    pub impl<T: Copy Send> PodFlattener<T> {
...
mod test {
    ...
    fn test_some_tcp_stream<U: Unflattener<int>, F: Flattener<int>>(
        ...
        let (begin_connect_chan, begin_connect_port) = pipes::stream();
        // The connection is sent from the server task to the receiver task
        // to handle the connection
        let (accept_chan, accept_port) = pipes::stream();
        ...
        do task::spawn |copy addr, move begin_connect_chan,
            ...
            let listen_res = do tcp::listen(
                ...
                let (res_chan, res_port) = pipes::stream();
